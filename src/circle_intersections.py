...
def get_linear_interpolation_of_circle(x: float, y: float, r: float, n: int):
...
def subdivide(points):
    ...
    r = [p_a]
    for p in points[1:]:
        p_n = subdivide_pair(p_a, p)
        r.append(p_n)
        r.append(p)
        p_a = p
    r.append(subdivide_pair(points[0], points[-1]))
    return r
